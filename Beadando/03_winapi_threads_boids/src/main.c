...
static void print_usage(const char* exe) {
    printf("Usage: %s [--mode seq|winapi] [--threads N] [--boids N] [--width W] [--height H] [--game peaceful|survival|terminate44]\n", exe);
    printf("       %s --benchmark N [--compare] [--mode seq|winapi] [--threads N] [--boids N] [--width W] [--height H] [--game peaceful|survival|terminate44]\n", exe);
...
static const char* run_mode_name(RunMode mode) {
    return mode == RUNMODE_SEQ ? "seq" : "winapi";
}
...
static bool benchmark_open_log_file(void) {
    ...
    snprintf(g_benchmarkLogPath, sizeof(g_benchmarkLogPath),
             ...
             (unsigned)st.wSecond);
#else
    snprintf(g_benchmarkLogPath, sizeof(g_benchmarkLogPath), "boids_winapi_benchmark_last.txt");
#endif
...
static bool startup_collect_values(HWND hwnd, StartupPromptState* state) {
    ...
    next.mode = next.threadCount <= 1 ? RUNMODE_SEQ : RUNMODE_WINAPI;
...
static bool prompt_startup_config(AppConfig* cfg, bool benchmarkPrompt) {
    ...
    const wchar_t* className = L"WinApiBoidsStartupPromptWindow";
    ...
    hwnd = CreateWindowExW(windowExStyle,
                           className,
                           benchmarkPrompt ? L"WinAPI boids mérés indítás" : L"WinAPI boids játék indítás",
...
static bool app_create_world(AppState* s) {
    ...
    if (s->cfg.mode != RUNMODE_SEQ) {
        if (!update_winapi_init(&s->updater, (size_t)s->cfg.threadCount)) {
...
static void app_destroy(AppState* s) {
    ...
    if (s->updaterReady) {
        update_winapi_destroy(&s->updater);
        s->updaterReady = false;
    }
    world_destroy(&s->world);
}
...
static void app_step_boids_winapi(AppState* s, double simDt) {
    update_winapi_step(&s->updater, &s->world, simDt);
}
...
static int run_compare_benchmark(const AppConfig* cfg, double simDt) {
    ...
    AppConfig winapiCfg = *cfg;
    AppState seqState;
    AppState winapiState;
    ...
    winapiCfg.mode = RUNMODE_WINAPI;

    if (!app_prepare_benchmark_state(&seqState, seqCfg, benchmarkSeed)) {
        return 1;
    }
    if (!app_prepare_benchmark_state(&winapiState, winapiCfg, benchmarkSeed)) {
        app_destroy(&seqState);
        return 1;
    }

    seqResult = app_run_benchmark(&seqState, seqCfg.benchmarkWarmup, seqCfg.benchmarkSteps, simDt);
    winapiResult = app_run_benchmark(&winapiState, winapiCfg.benchmarkWarmup, winapiCfg.benchmarkSteps, simDt);

    if (winapiResult.avgMs > 0.0) {
        speedup = seqResult.avgMs / winapiResult.avgMs;
    }

    snprintf(text, sizeof(text),
             "benchmark compare game=%s section=world_update_only boids=%d size=%dx%d steps=%d\n"
             "  seq:        avg=%.3f ms/tick | ticks=%.2f/s\n"
             "  winapi(%d): avg=%.3f ms/tick | ticks=%.2f/s\n"
             "  speedup:    %.2fx\n",
             game_mode_name(cfg->gameMode),
             cfg->boidCount,
             cfg->width,
             ...
             winapiCfg.threadCount,
             winapiResult.avgMs,
             winapiResult.ticksPerSecond,
             speedup);

    benchmark_write_text(cfg, text);

    app_destroy(&seqState);
    app_destroy(&winapiState);
    return 0;
}
    ...
    if (argc <= 1) {
        ...
        for (int i = 1; i < argc; i++) {
            ...
            if (strcmp(argv[i], "--mode") == 0 && i + 1 < argc) {
                ...
                else if (strcmp(m, "winapi") == 0) cfg.mode = RUNMODE_WINAPI;
                else {
